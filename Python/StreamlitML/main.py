...
###################### Section 7 ########################
# A Dashboard to look at the Quarterly population of Canada

# Challenges 
# Users selecting date not in the dataset(warning)
# When start date is greater than end date
# Dynamic text based on selected items
...
# 3. Expander (collapsible emement for more information)
...
@st.cache_data  #Catching imported data
def read_data():
...
with st.form("population-form"):
    ...
    with col1:
        ...
        start_quarter = st.selectbox("Quarter", options=['Q1','Q2','Q3','Q4'], index=2, key="start_q") # The key enable us to separate our selectboxes
        start_year = st.slider("Year", min_value= 1991, max_value= 2023, value=1991, step=1, key= "start_y")
    
    with col2:
        st.write("Choose an end date")
        end_quarter = st.selectbox("Quarter", options=['Q1','Q2','Q3','Q4'], index=0, key="end_q") # The key enable us to separate our selectboxes
        end_year = st.slider("Year", min_value= 1991, max_value= 2023, value=2023, step=1, key= "end_y")
    
    with col3:
        st.write("Choose a locatiion")
...
#Function to format date for eazy comparison with the table
@st.cache_data
...
if start_date not in df['Quarter'].tolist() or end_date not in df['Quarter'].tolist():
    st.error("No data available. Check your quarter and year selected") #Q1 1991
elif end_before_start(start_date, end_date):
    st.error("Date don't work. Start date must come before end date.")
else:
